...
const STATUS_CONTENT_MAP = {
  ...
  processing: {
    text: "Płatność jest w trakcie przetwarzania.",
    iconColor: "#6D6E78",
    icon: InfoIcon,
  },
  requires_payment_method: {
    text: "Płatność nieudana.",
    iconColor: "#DF1B41",
    icon: ErrorIcon,
  },
  default: {
    text: "Podczas płatności wystąpił błąd.",
    iconColor: "#DF1B41",
    icon: ErrorIcon,
  },
};
...
export default function Complete() {
  ...
  return (
    <div className="mt-20 max-w-screen-xl mx-3 sm:mx-auto">
      <Card id="payment-status" className="max-w-lg">
        <CardHeader>
          <CardTitle className="text-center">
            <div
              id="status-icon"
              className="w-fit rounded-full p-3 mx-auto"
            ...
            <h2 id="status-text" className="mt-3">{STATUS_CONTENT_MAP[status as keyof typeof STATUS_CONTENT_MAP].text}</h2>
          </CardTitle>
        </CardHeader>
        {status === "succeeded" && <CardContent>
          <CardDescription className="text-center">
            Dziękujemy za zakupy w naszym sklepie. Twój identyfikator zamówienia to: <strong>{intentId}</strong>.
          </CardDescription>
        </CardContent>}
      </Card>
    </div>
